...
function CustomersContent() {
    ...
    const handleEditCustomer = useCallback((c: Customer) => { setSelectedCustomer(c); setIsCustomerDialogOpen(true); }, []);
    const handleDeleteCustomer = useCallback((c: Customer) => { setSelectedCustomer(c); setIsDeleteDialogOpen(true); }, []);
    ...
    useKeyboardShortcuts([{ key: 'F3', action: () => searchInputRef.current?.focus(), description: 'Chercher', ignoreInputFocus: true }, { key: 'n', action: () => { setSelectedCustomer(null); setIsCustomerDialogOpen(true); }, description: 'Nouveau', ignoreInputFocus: false }], 'Clients');
        ...
        <div className="p-4 sm:p-6 space-y-6 max-w-[1800px] mx-auto">
            ...
            <div className="flex flex-col lg:flex-row gap-3">
                <div className="relative flex-grow">
                    ...
                    <Input ref={searchInputRef} placeholder="Rechercher... [F3]" className="pl-10 h-11 rounded-xl bg-card border-none shadow-sm" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
                <div className="flex gap-2">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild><Button variant="outline" className="rounded-xl h-11 bg-card border-none">{sortOptions[sortBy]}</Button></DropdownMenuTrigger>
                        <DropdownMenuContent className="rounded-xl border-none shadow-xl">
                            <DropdownMenuRadioGroup value={sortBy} onValueChange={setSortBy}>
                                {Object.entries(sortOptions).map(([k, v]) => <DropdownMenuRadioItem key={k} value={k} className="text-xs font-bold">{v}</DropdownMenuRadioItem>)}
                    ...
                    {isFiltered && <Button variant="ghost" size="icon" className="h-11 w-11 rounded-xl text-destructive" onClick={resetFilters}><FilterX className="h-4 w-4" /></Button>}
                </div>
            </div>
            ...
            <div className="min-h-[450px]">
               {isLoading ? (
                    ...
                    <EmptyState icon={Users} title="Aucun client" description={isFiltered ? "Ajustez les filtres." : "Ajoutez un client."} />
               ) : viewMode === 'list' ? (
                    <CustomerTable customers={customers} onEdit={handleEditCustomer} onDelete={handleDeleteCustomer} selectedCustomers={selectedCustomers} onToggleSelection={u => setSelectedCustomers(p => { const n = new Set(p); if (n.has(u)) n.delete(u); else n.add(u); return n; })} onToggleSelectAll={() => setSelectedCustomers(p => p.size === customers.length ? new Set() : new Set(customers.map(c => c.uuid)))} />
               ) : (
