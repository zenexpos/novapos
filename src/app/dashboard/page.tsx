...
export default function DashboardPage() {
    ...
    const statCards = useMemo(() => [
        ...
        { title: 'Bénéfice', value: formatCurrency(data?.stats.netProfit ?? 0), icon: Percent, change: data?.stats.netProfitChange, color: 'emerald' as const },
    ...
    return (
        <div className="p-6 space-y-8 max-w-[1800px] mx-auto animate-in fade-in duration-1000 pb-20">
            <PageHeader
                ...
                description="Centre de commandement souverain iPOS Zen"
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    <Card className="rounded-2xl border-white/5 bg-primary/5 shadow-xl overflow-hidden group">
                        <CardHeader className="bg-primary/10 border-b border-primary/20 p-4">
                            <CardTitle className="text-xs font-black uppercase tracking-widest text-primary flex items-center gap-2">
                                <PlusCircle className="h-4 w-4" /> Actions Rapides
                            </CardTitle>
                        </CardHeader>
                    ...
                    <Card className="rounded-2xl border-white/5 bg-card/40 backdrop-blur-sm shadow-sm overflow-hidden">
                        <CardHeader className="bg-muted/20 border-b border-white/5 p-4">
                            <CardTitle className="text-xs font-black uppercase tracking-widest opacity-40">Santé du Stock</CardTitle>
                        </CardHeader>
                        <CardContent className="p-6 space-y-6">
                            <HealthMetric label="Ruptures" value={data?.inventoryHealth.outOfStock ?? 0} color="text-red-500" total={data?.inventoryHealth.outOfStock ?? 0 + (data?.inventoryHealth.healthy ?? 0)} />
                            <HealthMetric label="Stock Faible" value={data?.inventoryHealth.lowStock ?? 0} color="text-amber-500" total={data?.inventoryHealth.healthy ?? 0} />
                            <HealthMetric label="Opérationnels" value={data?.inventoryHealth.healthy ?? 0} color="text-emerald-500" total={data?.inventoryHealth.healthy ?? 0} />
                        </CardContent>
                    </Card>
                </div>
            </div>
...
function HealthMetric({ label, value, color, total }: any) {
    return (
