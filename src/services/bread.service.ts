...
class BreadService {
    ...
    async generateAndGetOrdersForDate(
        ...
        // FIX: Use filter instead of where().equals() for boolean fields. 
        // Boolean indexes are not supported in all IndexedDB implementations and can throw DataError.
        const activeBreadClientsCount = await db.customers
            .filter(c => !!c.isBreadClient)
            .count();
    ...
    private async createDayOrders(date: string): Promise<void> {
        ...
        if (ordersToCreate.length > 0) {
            // FIX: Prevent double generation by using a transaction
    ...
    async addManualBreadOrder(params: {
        ...
        if (customerUuid) {
            // FIX: Dexie where({}) requires composite index. Using separate where + and instead.
