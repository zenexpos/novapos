...
export const SalesHistoryTable = memo(({ 
    ...
    const statusMap = {
        paid: { text: 'Payé', className: 'bg-emerald-500/10 text-emerald-600 border-transparent' },
        partial: { text: 'Partiel', className: 'bg-amber-500/10 text-amber-600 border-transparent' },
        unpaid: { text: 'Crédit', className: 'bg-red-500/10 text-red-600 border-transparent' },
    };
            ...
            <Table>
                <TableHeader className="bg-muted/30">
                    <TableRow className="border-none h-7">
                        ...
                        <TableHead className="px-1 text-[7px] font-black uppercase text-muted-foreground/40 tracking-widest">Référence</TableHead>
                        <TableHead className="px-1 text-[7px] font-black uppercase text-muted-foreground/40 tracking-widest">Partenaire</TableHead>
                        <TableHead className="px-1 text-[7px] font-black uppercase text-muted-foreground/40 tracking-widest">Statut</TableHead>
                        <TableHead className="px-1 text-right text-[7px] font-black uppercase text-primary tracking-widest">Total</TableHead>
                            ...
                            >
                                ...
                                <TableCell className="px-1 py-0">
                                    <span className="text-[8px] font-mono font-bold opacity-20">
                                        {isSale ? item.data.invoiceNumber.slice(-6) : 'PAIE-CLIENT'}
                                    </span>
                                </TableCell>
                                <TableCell className="px-1 py-0">
                                ...
                                <TableCell className="px-1 py-0">
                                    {isCancelled ? (
                                        <span className="text-[6px] font-black uppercase opacity-20">Annulée</span>
                                    ) : isSale ? (
                                        ...
                                        <span className="text-[6px] font-black text-emerald-600 uppercase opacity-30 italic">Encaissement</span>
                                    )}
                                </TableCell>
                                    ...
                                    {isSale && !isCancelled && (
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-32 rounded-xl border-white/5 bg-card">
                                                <DropdownMenuItem onClick={() => onViewDetails(item.data)} className="text-[9px] font-black p-2 uppercase">Détails</DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => onPrint(item.data)} className="text-[9px] font-black p-2 uppercase">Imprimer</DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => onCancel(item.data)} className="text-destructive text-[9px] font-black p-2 uppercase">Annuler</DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    )}
                                </TableCell>
