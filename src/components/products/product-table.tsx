...
/**
 * Optimized Product Row with Memoization to prevent unnecessary re-renders.
 */
            ...
            <TableCell className="px-3 py-4" onClick={e => e.stopPropagation()}>
                {editingField === 'quantity' ? (
                    <Input 
                        ...
                        autoFocus
                        onBlur={(e) => { onQuickEdit(product.uuid, 'quantity', e.target.value); setEditingField(null); }}
            ...
            <TableCell className="px-3 py-4 text-right" onClick={e => e.stopPropagation()}>
                {editingField === 'price' ? (
                    <Input 
                        ...
                        autoFocus
                        onBlur={(e) => { onQuickEdit(product.uuid, 'price', e.target.value); setEditingField(null); }}
                        onKeyDown={(e) => e.key === 'Enter' && (e.target as HTMLInputElement).blur()}
                    />
                ) : (
                    <span 
            ...
            <Table>
                <TableHeader className="bg-muted/30">
                    <TableRow className="border-b border-white/5 hover:bg-transparent">
                        ...
                        <TableHead className={thCls}><button className={thBtn} onClick={() => toggleSort('name')}>Désignation <SortIcon col="name" sortKey={sortKey} sortDir={sortDir} /></button></TableHead>
                        <TableHead className={thCls}><button className={thBtn} onClick={() => toggleSort('stockStatus')}>État <SortIcon col="stockStatus" sortKey={sortKey} sortDir={sortDir} /></button></TableHead>
                        <TableHead className={cn(thCls, 'hidden lg:table-cell')}>Catégorie</TableHead>
                        <TableHead className={thCls}><button className={thBtn} onClick={() => toggleSort('quantity')}>Stock <SortIcon col="quantity" sortKey={sortKey} sortDir={sortDir} /></button></TableHead>
                        <TableHead className={cn(thCls, 'text-right hidden md:table-cell')}><button className={cn(thBtn, 'ml-auto')} onClick={() => toggleSort('purchasePrice')}>PMP <SortIcon col="purchasePrice" sortKey={sortKey} sortDir={sortDir} /></button></TableHead>
                        <TableHead className={cn(thCls, 'text-right')}><button className={cn(thBtn, 'ml-auto')} onClick={() => toggleSort('price')}>Prix Vente <SortIcon col="price" sortKey={sortKey} sortDir={sortDir} /></button></TableHead>
                        <TableHead className={cn(thCls, 'text-right hidden sm:table-cell')}><button className={cn(thBtn, 'ml-auto')} onClick={() => toggleSort('margin')}>Marge <SortIcon col="margin" sortKey={sortKey} sortDir={sortDir} /></button></TableHead>
