...
import { CircleCheckBig, CircleUser, Package, Landmark, Loader2, Trash2, User, Sparkles, Hash, Banknote } from 'lucide-react';
...
export function BreadOrderCard({ order, isSelected, onToggleSelection, onUpdate }: BreadOrderCardProps) {
    ...
    const togglePayment = async (checked: boolean) => {
        ...
        try {
            ...
            toast.error("Erreur lors du changement de statut de paiement.");
        } finally {
            setIsUpdatingStatus(false);
        }
    };
            ...
            <CardContent className="p-4 pt-2 space-y-4 relative z-10">
                ...
                <div className="p-3 bg-muted/20 rounded-xl border border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">Payé</span>
                    </div>
                    <Switch 
                        checked={isPaid} 
